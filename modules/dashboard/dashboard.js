...
export async function initDashboardModule(container, options = {}) {
  ...
  /*
   * IMPORTANTE:
   * O banco agora é Supabase/PostgreSQL.
   * Portanto, estas consultas precisam ser aguardadas.
   *
   * Não existe cache/localStorage de dados de negócio aqui.
   */
  const storeId = await db.getCurrentStoreId();
  const currentStore = await db.getCurrentStore();
  const stats = await db.getDashboardStats(storeId);
      ...
      <div class="metrics-grid">
        ...
        <div class="card metric-card">
          ...
          <div class="metric-value" style="color: #f8fafc;">
            ${stats.totalRealEquity.toLocaleString('pt-MZ', { minimumFractionDigits: 2 })}
            <span style="font-size: 14px; color: #ea580c;">MT</span>
          </div>

          <div class="metric-sub">
            Caixa:
            <strong style="color: #cbd5e1;">
              ${stats.currentCashInDrawer.toLocaleString('pt-MZ')} MT
            </strong>
            +
            Estoque Custo:
            <strong style="color: #cbd5e1;">
              ${stats.stockCostTotal.toLocaleString('pt-MZ')} MT
            </strong>
          </div>
        </div>
        ...
        <div class="card metric-card">
          ...
          <div class="metric-value" style="color: #34d399;">
            ${stats.totalTodaySales.toLocaleString('pt-MZ', { minimumFractionDigits: 2 })}
            <span style="font-size: 14px; color: #10b981;">MT</span>
          </div>
          ...
          <div class="metric-sub">
            ...
            ${stats.todaySalesCount === 1 ? 'venda concluída' : 'vendas concluídas hoje'}
          </div>
        </div>
        ...
        <div class="card metric-card">
          ...
          <div class="metric-value" style="color: #60a5fa;">
            ${stats.stockSaleValuation.toLocaleString('pt-MZ', { minimumFractionDigits: 2 })}
            <span style="font-size: 14px; color: #3b82f6;">MT</span>
          </div>
        ...
        <div class="card metric-card">
          ...
          <div class="metric-value" style="color: #fbbf24;">
            ${stats.totalReceivable.toLocaleString('pt-MZ', { minimumFractionDigits: 2 })}
            <span style="font-size: 14px; color: #f59e0b;">MT</span>
          </div>
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 14px;">
          ...
          <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <div>
              <div style="font-size: 13px; font-weight: 700; color: #f8fafc;">
                ${stats.lowStockCount + stats.outOfStockCount} Itens em Atenção
              </div>

              <div style="font-size: 11px; color: #94a3b8;">
                ${stats.outOfStockCount} zerados • ${stats.lowStockCount} abaixo do mínimo
              </div>
            </div>
          </div>
        ...
        <div class="card" style="padding: 14px 18px; display: flex; align-items: center; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <div>
              <div style="font-size: 13px; font-weight: 700; color: #f8fafc;">
                ${stats.pendingQuotesCount} Orçamentos Abertos
              </div>

              <div style="font-size: 11px; color: #94a3b8;">
                Cotações de obras em acompanhamento
              </div>
            </div>
          </div>
        ...
        <div class="card" style="padding: 14px 18px; display: flex; align-items: center; justify-content: space-between;">
          <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <div>
              <div style="font-size: 13px; font-weight: 700; color: #f8fafc;">
                ${stats.pendingDeliveriesCount} Cargas em Logística
              </div>

              <div style="font-size: 11px; color: #94a3b8;">
                Despachos e entregas em canteiro
              </div>
            </div>
          </div>
              ...
              <tbody>
                ...
                ` : stats.recentSales.map(sale => `
                  <tr>
                    ...
                    <td>
                      <strong style="color: #34d399; font-family: var(--font-mono);">
                    ...
                    <td style="text-align: right;">
                      ...
                      <button
                        class="btn btn-secondary btn-print-recent"
            ...
            ` : stats.topSellingProducts.map((p, idx) => `
              <div style="display: flex; align-items: center; justify-content: space-between; gap: 8px;">
                ...
                <div style="display: flex; align-items: center; gap: 10px;">
                  ...
                  <span style="font-weight: 900; color: #ea580c; font-size: 13px; width: 16px;">
                    #${idx + 1}
                  </span>
  ...
  const quickPos =
    container.querySelector('#btn-quick-pos');

  if (quickPos) {
    quickPos.onclick = () => onNavigate?.('PDV');
  }


  const quickQuote =
    container.querySelector('#btn-quick-quote');

  if (quickQuote) {
    quickQuote.onclick = () => onNavigate?.('ORCAMENTOS');
  }


  const quickStock =
    container.querySelector('#btn-quick-stock');

  if (quickStock) {
    quickStock.onclick = () => onNavigate?.('ESTOQUE');
  }


  const stockAlert =
    container.querySelector('#btn-goto-stock-alert');

  if (stockAlert) {
    stockAlert.onclick = () => onNavigate?.('ESTOQUE');
  }


  const quotesAlert =
    container.querySelector('#btn-goto-quotes-alert');

  if (quotesAlert) {
    quotesAlert.onclick = () => onNavigate?.('ORCAMENTOS');
  }


  const deliveriesAlert =
    container.querySelector('#btn-goto-deliveries-alert');

  if (deliveriesAlert) {
    deliveriesAlert.onclick = () => onNavigate?.('ENTREGAS');
  }


  const viewAllSales =
    container.querySelector('#btn-view-all-sales');

  if (viewAllSales) {
    viewAllSales.onclick = () => onNavigate?.('VENDAS');
  }


  container
    .querySelectorAll('.btn-print-recent')
    .forEach(btn => {
